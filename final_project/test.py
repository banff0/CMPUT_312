...
def preprocess_str(str):
        ...
        for i in range(1, len(words)):
            word = words[i]
            if "\n" in word:
                new_line = word.split()[-1]
                line_len = len(new_line) + 1    
                words[i] += "_"
                continue
            if line_len + len(word) + 1 >= 10:
                words[i - 1] += "\n"
                if line_len > len(words[i - 1]) + 1:
                    words[i-1] = "_" + words[i-1]
                line_len = len(word) + 1
                continue
            print(line_len, len(words[i - 1]) + 1)
            print(words[i-1], words[i])
            if line_len > len(words[i - 1]) + 1:
                print("SPACE", "_" + words[i-1])
                words[i-1] = "_" + words[i-1]
            line_len += len(word) + 1
...
print(preprocess_str("ABCD EFG HIJ KLO sdl ou ouw i ij ijs\nisn i iis isdn \ns\nsisnd"))